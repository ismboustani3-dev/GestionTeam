      ...
      if (newScheduleTime2) {
        ...
        const res = await fetch('/api/schedule', {
          ...
          body: JSON.stringify({
            action: 'add',
            schedules: [
              { name: `${name} (1)`, type: newScheduleType, cronExpression: `${parseInt(m1)} ${parseInt(h1)} * * ${dayStr}`, teamName: newScheduleTeam },
              { name: `${name} (2)`, type: newScheduleType, cronExpression: `${parseInt(m2)} ${parseInt(h2)} * * ${dayStr}`, teamName: newScheduleTeam }
            ]
          })
        });
        const data = await res.json();
